...
map("n", "<leader>ms", "<cmd>mksession! ~/.config/nvim/sessions/session.vim<CR>")
map("n", "<leader>e", vim.cmd.Ex)
map("n", "tn", vim.cmd.tabnext)
map("n", "tp", vim.cmd.tabprevious)
map("n", "<A-tab>", vim.cmd.tabnext)
map("n", "<AS-tab>", vim.cmd.tabprevious)
map("n", "<Tab>", "<C-w>w")
map("n", "<C-w><C-t>", "<cmd>tab sp<CR>")
map("v", "<C-j>", "<cmd>m '>+1<CR>gv=gv", { desc = "moves lines down in visual selection" })
map("v", "<C-k>", "<cmd>m '<-2<CR>gv=gv", { desc = "moves lines up in visual selection" })
